...
export class ReactApp extends Construct {
  ...
  constructor(
    ...
    frontendBuildPath =
      frontendBuildPath || path.resolve(frontendPath, "build");
    ...
    if (!doNotBuild) {
      try {
        ...
        throw new Error(
          `There was a problem building the "${this.node.id}" ReactApp.`
        );
      }
    }
    ...
    this.webAppBucket = new s3.Bucket(this, `${bucketName}_Bucket`, {
      ...
      blockPublicAccess: withCloudfrontDistribution
    ...
    if (withCloudfrontDistribution) {
      const cloudfrontOAIforUserApp = new cf.OriginAccessIdentity(
        this,
        "cloudfront-OAI",
        {
        ...
        new iam.PolicyStatement({
          ...
          principals: [
            new iam.CanonicalUserPrincipal(
              cloudfrontOAIforUserApp.cloudFrontOriginAccessIdentityS3CanonicalUserId
            ),
          ],
        })
      );
      ...
      const certificate =
        ...
        cm.Certificate.fromCertificateArn(
          this,
          "Certificate",
          domain.domainCertificateArn
        );

      this.webDistribution = new cf.CloudFrontWebDistribution(
        this,
        `${bucketName}_Distribution}`,
        {
          viewerCertificate:
            domain &&
            certificate &&
      ...
      const record =
        ...
        new r53.ARecord(this, `${domain.domainName}_Alias}`, {
          ...
          target: r53.RecordTarget.fromAlias(
            new r53t.CloudFrontTarget(this.webDistribution)
          ),
          zone: domain.hostedZone,
        });
    }

    const webAppDeploymentAssets = new s3d.BucketDeployment(
      this,
      `${bucketName}_Deployment_Assets`,
      {
    ...
    const webAppDeploymentIndex = new s3d.BucketDeployment(
      this,
      `${bucketName}_Deployment_Index`,
      {
        destinationBucket: this.webAppBucket,
        sources:
          (!doNotUpload && [
